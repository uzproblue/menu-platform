...
/** Same host as menu-server R2 public objects; required by `image-loader.ts` to rewrite absolute R2 URLs to `/cdn-cgi/image/...` on that zone. Inlined at build — if CI only sets `R2_PUBLIC_BASE_URL`, forward it so the client bundle still matches `src` URLs. */
