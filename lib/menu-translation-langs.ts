 ...
 */
export const LOCATION_TRANSLATION_OPTIONS = ["UZ", "KZ", "RU", "EN", "KGZ"] as const;

/** Default selection for new locations / fallback when API returns none (subset of allowed codes). */
export const DEFAULT_LOCATION_TRANSLATION_SELECTION = ["EN", "RU", "UZ"] as const;
 ...
 */
export function validateTranslationLangsInput(codes: string[]):
  ...
  if (unique.length === 0) {
    return {
      ok: false,
      message: "translationLangs must contain at least one allowed language code",
    };
  }
