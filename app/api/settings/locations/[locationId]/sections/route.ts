...
import { updateLocationSectionsWithAuthServer } from "@/lib/auth-api";
import {
...
export async function PATCH(
  ...
  for (const x of rawIds) {
    ...
    sectionIds.push(x.trim());
  }

  const restaurantId = await getSelectedRestaurantIdFromCookies();
  const result = await updateLocationSectionsWithAuthServer(
    token,
    trimmedId,
    { sectionIds },
    restaurantId,
  ...
  if (!exportResult.ok) {
    console.error(
      "[PATCH location sections] location public export failed",
      trimmedId,
  ...
  void trackStaffMutation(PlatformEvent.LOCATION_SECTIONS_ASSIGNED, {
