...
import { batchReassignCategoriesWithAuthServer } from "@/lib/auth-api/catalog-categories";
...
export async function POST(req: Request) {
  ...
  const targetSectionId = typeof o.targetSectionId === "string" ? o.targetSectionId.trim() : "";
  const categoryIds = Array.isArray(o.categoryIds)
    ? o.categoryIds.filter((id): id is string => typeof id === "string" && id.trim().length > 0)
    : [];

  if (!targetSectionId || categoryIds.length === 0) {
    return NextResponse.json(
      { error: "invalid_body", message: "targetSectionId and non-empty categoryIds are required" },
      { status: 400 },
    );
  }

  const restaurantId = await getSelectedRestaurantIdFromCookies();
  const result = await batchReassignCategoriesWithAuthServer(
    token,
    categoryIds,
    targetSectionId,
    restaurantId,
  ...
  if (!exportBatchResult.ok) {
    console.error(
      "[POST categories batch-reassign] restaurant location export batch failed",
