...
**Global menu item/category renames:** PATCH returns immediately after the D1 update. Gemini re-translation runs in a **debounced** background pipeline (`schedulePostCatalogChangePipeline`, ~3s) when the name/description changed **or** the row still has zero guest translation rows (e.g. after iiko import). The debounce wait and pipeline must run inside **`ctx.waitUntil`** on Cloudflare Workers so the isolate stays alive after the HTTP response. Translation sync calls **menu-server** `POST …/sync-translations`, which needs **`CF_AI_GATEWAY_BASE_URL`** and **`CF_AI_GATEWAY_API_KEY`** on the menu-server Worker (check logs for `CF_AI_GATEWAY not configured`). Guest menus update shortly after that pipeline finishes; do not retry saves on 502 from the old synchronous translation path.
