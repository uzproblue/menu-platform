...
**Global menu item/category renames:** PATCH returns immediately after the D1 update. Gemini re-translation (when name or description changed) and full-restaurant R2 export run in a **debounced** background pipeline (`schedulePostCatalogChangePipeline`, ~3s). The debounce wait and pipeline must run inside **`ctx.waitUntil`** on Cloudflare Workers so the isolate stays alive after the HTTP response; a bare `setTimeout` after the response will not run in production. Guest menus update shortly after that pipeline finishes; do not retry saves on 502 from the old synchronous translation path.
